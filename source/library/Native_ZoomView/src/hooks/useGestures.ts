...
export const useGestures = ({
  ...
  const handleScale: (value: any) => void = (value) => {
    handlePinch?.(value)
  }
  ...
  const pinchGesture = Gesture.Pinch()
    ...
    .onEnd((...args) => {
      ...
      runOnJS(handleScale)({ scale: scale.value, savedScale: savedScale.value })
    });
  ...
  return { gestures, animatedStyle, reset, scale: scale.value, savedScale: savedScale.value };
};
